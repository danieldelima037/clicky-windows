...
interface OpenCodeZenQueryParams {
  transcript: string;
...
export class OpenCodeZenService {
  ...
  async query(params: OpenCodeZenQueryParams): Promise<OpenCodeZenResponse> {
    const apiKey = this.settings.get("opencodeZenApiKey");
    const model = this.settings.get("opencodeZenModel");
    ...
    if (!response.ok) {
      ...
      throw new Error(`OpenCode Zen API error (${response.status}): ${error}`);
    }
